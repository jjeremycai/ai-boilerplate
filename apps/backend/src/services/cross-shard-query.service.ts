...
export class CrossShardQueryService {
  ...
  async executeDistributedTransaction(
    ...
    const shardPromises = Array.from(operationsByShard.entries()).map(
      async ([shardId, shardOps]) => {
        try {
          const shard = this.router.getShardById(shardId);
          if (!shard) {
            throw new Error(`Shard ${shardId} not found`);
          }

          // Execute all operations for this shard in a transaction
          const shardResults = await shard.db.batch(
            shardOps.map(op => shard.db.prepare(op.query).bind(...op.params))
          );
    ...
    for (const shard of shards) {
      ...
      while (hasMore) {
        ...
        try {
          ...
          console.error(`Error streaming from shard ${shard.id}:`, error);
          hasMore = false;
        }
      }
    }
  ...
  private sortResults<T>(results: T[], orderBy: { column: string; direction: 'ASC' | 'DESC' }): T[] {
  ...
  private async determineShardForOperation(
    operation: { query: string; params: unknown[] }
  ): Promise<string> {
    ...
    if (idMatch && idMatch[1]) {
      const universalId = require('../lib/universal-id');

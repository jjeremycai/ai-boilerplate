...
export class ShardReferenceTracker {
   ...
   */
  async validateReferences(table: string, id: string): Promise<{
    valid: boolean;
   ...
   */
  async getRelatedDataDistribution(table: string, id: string): Promise<{
  ...
  private getShardFromId(id: string): string {
    try {
      const UniversalIdGenerator = require('../lib/universal-id');
  ...
  async exportReferenceData(): Promise<{
    ...
    for (const [key, references] of this.referenceCache) {
      ...
      for (const ref of references) {
        ...
        referencesByType[ref.referenceType] = 
          (referencesByType[ref.referenceType] || 0) + 1;
      }
    }

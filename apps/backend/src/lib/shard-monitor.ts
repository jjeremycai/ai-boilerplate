...
export class ShardMonitor {
  ...
  async checkShardHealth(shardId: string, db: D1Database): Promise<ShardHealthReport> {
    ...
    for (const table of tables.results) {
      ...
      const tablePages = await db.prepare(`
    ...
    const largestTable = tablesInfo.reduce((prev, current) => 
      prev.recordCount > current.recordCount ? prev : current
    , tablesInfo[0]);

    if (largestTable && largestTable.recordCount > totalRecords * 0.7) {
      recommendations.push(`Table '${largestTable.tableName}' contains ${((largestTable.recordCount / totalRecords) * 100).toFixed(1)}% of records. Consider partitioning.`);
    }
  ...
  async getShardAllocationReport(): Promise<{
    ...
    const warningShards = reports
      .filter(r => r.healthStatus === 'warning')
      .map(r => r.shardId);
